...
async function parseJsonResponse(res) {
  ...
  if (!contentType.includes("application/json")) {
    if (res.status === 504 || res.status === 502) {
      throw new Error(
        "The server took too long to respond (likely timed out checking a large collection against Scryfall). Please try again - it sometimes succeeds on a retry."
      );
    }
    throw new Error(
      `Server returned an unexpected response (status ${res.status}) instead of data. This usually means the request failed or timed out server-side.`
    );
  }
