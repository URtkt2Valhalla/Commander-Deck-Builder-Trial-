...
export default async function handler(req, res) {
  ...
  try {
    const { map: details, failedBatches, totalBatches } = await getCardDetails(cardNames);
    const commanders = [];
    ...
    return res.status(200).json({
      commanders,
      checked: cardNames.length,
      warning:
        failedBatches > 0
          ? `${failedBatches} of ${totalBatches} Scryfall lookups failed even after retries - some commanders may be missing from this list due to a temporary Scryfall issue. Try again in a bit.`
          : null,
    });
  } catch (err) {
