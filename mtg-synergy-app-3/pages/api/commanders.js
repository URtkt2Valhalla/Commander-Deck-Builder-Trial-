...
export default async function handler(req, res) {
  ...
  try {
    const { map: details, failedBatches, totalBatches, sampleErrors } = await getCardDetails(cardNames);
    const commanders = [];
    ...
    return res.status(200).json({
      ...
      warning:
        failedBatches > 0
          ? `${failedBatches} of ${totalBatches} Scryfall lookups failed even after retries - some commanders may be missing. Details: ${sampleErrors.join(
              " | "
            )}`
          : null,
    });
