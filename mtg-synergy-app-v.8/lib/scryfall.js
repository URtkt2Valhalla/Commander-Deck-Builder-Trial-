...
// How many batches to have in flight at once, paired with retries below.
// Scryfall's guidance is to stay under 10 requests/second - bursty
// concurrency here was previously causing some batches to get silently
// rate-limited (429) and their cards dropped with no visible error, which
// is why individual commanders could randomly go missing from results.
const CONCURRENCY = 4;
const MAX_RETRIES = 3;
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (res.status === 429 || res.status >= 500) {
        ...
        // instead of silently dropping this batch's cards.
